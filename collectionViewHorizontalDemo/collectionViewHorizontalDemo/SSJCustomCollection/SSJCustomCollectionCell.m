...
@implementation SSJCustomCollectionCell
...
- (void)refurbishImageWithAdd{
    ...
    if (self.lastSuperView) {
        ...
        //        UITapGestureRecognizer *tagDouble=[[UITapGestureRecognizer alloc]initWithTarget:self.lastSuperView action:@selector(doubleTap:)];
...
- (void)refurbishUIWithModel:(SSJCustomCollectionCellModel *)model everyLineCount:(NSInteger)everyLineCount{
    ...
    if (model.describeStr.length > 0) {
        self.name_label.text = model.describeStr;
    }
